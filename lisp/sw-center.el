...
Margins are removed when the frame has side-by-side windows."
  ...
  (if sw-center-mode
      (progn
        (add-hook 'window-configuration-change-hook #'sw-center--update)
        (sw-center--update))
    (remove-hook 'window-configuration-change-hook #'sw-center--update)
    (sw-center--clear)))
