...
(defvar sw-dashboard--os-version nil
  "Cached OS version string, computed on first render.
Lazy because the macOS branch shells out to sw_vers, which would
otherwise block startup.")

(defun sw-dashboard--os-version ()
  "Return the OS version string, computing and caching it on first call."
  (or sw-dashboard--os-version
      (setq sw-dashboard--os-version
            (pcase system-type
              ('gnu/linux
               (or (when-let* ((release
                                (ignore-errors
